defmodule SocialScribeWeb.ModalComponents do
  ...
  Follows the same structure as `hubspot_modal/1` so the Salesforce modal
  benefits from the same accessibility and animation behaviour.

  ## Examples

      <.salesforce_modal id="sf-modal" show on_cancel={JS.patch(~p"/back")}>
        Modal content here
      </.salesforce_modal>
  """
  attr :id, :string, required: true
